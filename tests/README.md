...
- 核心 15 项：`weapon_choreography`（50；渲染时 52）、`atlas_pipeline`（55）、`arrow_attachment`（34；渲染时 40）、`character_art`（33）、`locomotion_art`（60；渲染时 62）、`rock_collision`（290）、`guard_reaction`（28）、`attack_motion`（15）、`battlefield`（17）、`guard_encounter`（28）、`mission`（29）、`short_level`（21）、`camp_loop`（22）、`space_combat`（27）、`combat_polish`（15）。共 724 项，渲染模式额外检查 10 项；这是套件规模，不代表每轮全跑。
...
- 22 个功能测试位于 `tactical/`。迁移路径和背包 view 归属已更新；`outpost_sample` 原本就有两项失败，已对照备份复现，并修正登台所需帧数与平台射线取样位置；该历史修正未改变几何、碰撞或断言阈值。
