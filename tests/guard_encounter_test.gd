...
func run() -> void:
	...
	for i in 3: preload("res://scripts/tactical/space_trace.gd").apply_cloth(cloth)
	await frames(3)
	...
	for pose in 5:
		var art: Image = preload("res://scripts/tactical/directional_art.gd").texture(2,-1,false,false,2,pose).get_image()
