...
环境 F2 粗像素对照属于显示阶段，**不改变角色烘焙规格或源资产**。角色、武器在环境颜色降采样后按原精度绘制，保留深度；不要为配合环境粗化而降低人物图集或重新烘焙。具体约定见 ENVIRONMENT_ART.md 与 ARCHITECTURE.md。
...
三种新物种分别增加哥布林 2、石头人 5、史莱姆 2 页对；七种合计 32 页对，全部同时载入时原始 RGBA8 约 256 MiB。实际场景只加载使用的种类；角色运行时没有逐帧图像回读或烘焙视口。
