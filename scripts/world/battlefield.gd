...
## 携信后显示返营提示；其余返回空格，以抑制试验场的默认路线文案。
...
## 用营火、铺盖和补给标记出生营地；交互规则由 objective 负责。
func camp() -> void:
...
## 按固定种子生成边缘岩石，使重复启动时外形稳定。
func edge_rock(point: Vector3, size: Vector3, seed_value: int) -> void:
	var prop=preload("res://scripts/world/battle_prop.gd").new()
	prop.position=point
	prop.extent=size
