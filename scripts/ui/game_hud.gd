...
func setup(caption: String) -> void:
	...
	panel.add_theme_stylebox_override("panel",preload("res://scripts/ui/pixel_style.gd").box())
	...
	add_child(status)
	notice = Label.new()
	notice.text = "WASD 移动 · Shift 疾跑（轻/中型） · 空格跳跃 · C 下蹲稳弓 · 左键挥刀 · 按住右键格挡 · E 交互 · I 背包 · 1–5 换装 · Alt 翻滚\nCtrl 精确射击 · R 重新出发（保留装备） · Esc 退出 · F11 全屏/窗口 · F1 标注 · F2 环境精度 · B 明暗对比"
	notice.position = Vector2(16, 560)
	...
	add_child(feedback)
	for label in [title,status,notice,feedback]:
