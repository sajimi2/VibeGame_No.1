...
func _init() -> void: cell_size=Vector2i(64,64)
...
func sample(animation: String, direction: int, phase: int, settings: Dictionary) -> Dictionary:
	...
	if part!="full":
		...
		var override:=Store.lookup(asset_id,key)
	...
	for layer in ["lower","upper"]:
		...
		var offset:=Vector2i(roundi(delta.x/Spec.PIXEL),roundi(-delta.y/Spec.PIXEL))
		var frame:=layer_image(key)
		var override:=Store.lookup(asset_id,key)
		...
		for y in 64:
			for x in 64:
				...
				if not Rect2i(0,0,64,64).has_point(at): continue
				var encoded:=depth.get_pixel(x,y)
				var z:=(encoded.r*65280+encoded.g*255)/65535*6-3+delta.z
				if z<=buffer[at.y*64+at.x]: continue
				buffer[at.y*64+at.x]=z
...
func validate_edit(document: RefCounted) -> String:
	...
	for index in document.cells.size():
		...
		var at:=Vector2i(index%12,index/12)*64
		for y in 64:
			for x in 64:
...
static func _project(point: Vector3, direction: int) -> Vector2:
	var p:=Basis(Vector3.RIGHT,deg_to_rad(Spec.PITCH)).inverse()*Basis(Vector3.UP,direction*PI/6)*point
	return Vector2(32+p.x/Spec.PIXEL,32-p.y/Spec.PIXEL)
